...
def check_salary_anomaly(data: dict) -> dict:
    ...
    company_claimed = data.get("company")
    
    if salary_offered is None: return _skip("No salary provided")

    lo, hi = SALARY_RANGES.get("default", (500000, 3500000))
    is_big_tech = company_claimed in ["google", "microsoft", "amazon", "apple", "meta", "infosys", "tcs"]
    
    effective_hi = hi * 4 if is_big_tech else hi * 2
    
    if salary_offered > effective_hi:
        return _flag(20, "Salary is significantly above market max", "SALARY_RISK", 0.85)
    return _clean("Salary within range", "SALARY_RISK")
...
def check_offer_text(data: dict) -> dict:
    ...
    for tier, config in SCAM_TEXT_TIERS.items():
        ...
        if hits:
            penalty = config["penalty"] * (1 + (len(hits)-1)*0.2)
...
def run_cyber_checks(job_url, recruiter_email, company_claimed):
    sample = {
        "job_url": job_url,
